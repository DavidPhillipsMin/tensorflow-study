...
flags.DEFINE_integer('batch_size', 10, 'Batch size per loop')

sess = tf.InteractiveSession()

chart = tf.placeholder(tf.float32, [None, ts.CHART_SIZE, 7, 1])
...
with tf.name_scope("input_layer"):
    w0 = tf.get_variable("input_layer_weight", shape=[3*7*128, 256], initializer=xavier_init(3*7*128, 256))
...
for repeats in range(FLAGS.training_repeat):
    ...
    for batch_set in range(100):
        ...
        avg_loss += loss.eval({chart: batch_charts, ratio: batch_labels, dropout_rate: FLAGS.dropout_rate, batch_set_size: FLAGS.batch_size}) / 5 #total_batch
...
# Test trained model
ts.shuffle_batch_set()
test_charts, test_labels = ts.next_batch(50, True)

#correct_prediction = tf.equal(tf.argmax(prediction, 1), tf.argmax(test_labels, 1))
correct_prediction = tf.less_equal(tf.abs(tf.argmax(prediction, 1) - tf.argmax(test_labels, 1)), 1)
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print(accuracy.eval({chart: test_charts, ratio: test_labels, dropout_rate: 1, batch_set_size: 50}))
