...
chart = tf.placeholder(tf.float32, [None, 21660, 1, 7])
tf.histogram_summary("chart", chart)

ratio = tf.placeholder(tf.float32, [None, 31])
tf.histogram_summary("label", ratio)

dropout_rate = tf.placeholder(tf.float32)
batch_set_size = tf.placeholder(tf.int32)

cw0 = tf.Variable(tf.random_normal([3, 1, 7, 32], stddev=0.01))
...
with tf.name_scope("output_layer"):
    w4 = tf.get_variable("output_layer_weight", shape=[256, 31], initializer=xavier_init(256, 31))
    #tf.histogram_summary("weight", w4)    
    b4 = tf.Variable(tf.random_normal([31]))
